...
def test_create():
    ...
    sample = smfsb.sim_sample(rng, 20, sir.m, 0, 20, step_sir)
...
def test_bd():
    ...
    out = smfsb.sim_time_series(rng, bd.m, 0, 100, 0.1, step)
...
def test_sim_time_series():
    bd = smfsb.models.bd()
    step = bd.step_gillespie()
    out = smfsb.sim_time_series(rng, bd.m, 0, 5, 1, step)
...
def test_dimer():
    ...
    out = smfsb.sim_time_series(rng, dimer.m, 0, 100, 0.1, step)
...
def test_id():
    ...
    out = smfsb.sim_time_series(rng, id.m, 0, 100, 0.1, step)
    assert out.shape == (1000, 1)
    assert out[100, 0] >= 0
...
def test_lv2():
    ...
    out = smfsb.sim_time_series(rng, lv.m, 0, 100, 0.1, step_lv)
    assert out.shape == (1000, 2)
    assert out[600, 1] >= 0
...
def test_discretise():
    dt = 0.01
    lv = smfsb.models.lv()
    times, states = lv.gillespie(rng, 2000)
...
def test_frm():
    ...
    out = smfsb.sim_time_series(rng, lv.m, 0, 10, 0.01, step_lv)
    assert out.shape == (1000, 2)
    assert out[600, 1] >= 0
...
def test_gillespied():
    lv = smfsb.models.lv()
    states = lv.gillespied(rng, 30, 0.1)
...
def test_mm():
    ...
    out = smfsb.sim_time_series(rng, mm.m, 0, 100, 0.1, step)
...
def test_sir():
    ...
    out = smfsb.sim_time_series(rng, sir.m, 0, 100, 0.1, step)
